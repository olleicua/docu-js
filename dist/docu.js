(function () {
  ...
  function addChildrenToElement($appendable, children) {
    if (!lodashExports.isArrayLike(children)) {
  ...
  function normalizePropertyName(key) {
    if (lodashExports.includes(alwaysLowerCasePropertyNames, key.toLowerCase())) {
  ...
  function assign(object, nestedProperties) {
    if (!lodashExports.isPlainObject(nestedProperties)) {
      throw 'Entity can only be assigned using a plain object';
    }

    const propertyKeys = lodashExports.keys(nestedProperties);
    let i, key, value;
    for (i = 0; i < propertyKeys.length; i++) {
      ...
      } else if (lodashExports.isObject(object[key])) {
  ...
  function append(parent, child) {
    ...
    } else if (lodashExports.isString(child)) {
